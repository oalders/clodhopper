...
USAGE
  ...
  clodhopper serve [--port N] [--host H | --tailscale] [--enable-merge]   serve the dashboard (default 127.0.0.1; --host 0.0.0.0 for container/LAN; --tailscale binds the Tailscale IPv4; --enable-merge turns on the roster's PR-action buttons, incl. rebase onto the default branch + force-push, and is restricted to loopback + Tailscale peers whatever --host says)
